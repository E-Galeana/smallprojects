...
#define TXTEDITOR_VERSION "0.0.1"
...
struct editorConfig {
    ...
    int numrows;
    erow *row;
    struct termios orig_termios; // original terminal attributes stored in a global variable.
};
...
void enableRawMode() {
    ...
    atexit(disableRawMode); // calls disableRawMode automatically when program exits.
...
int getCursorPosition(int *rows, int *cols) {
    ...
    example response: "\x1b[24;80R" (cursor at row 24, column 80.)
    */
      ...
      if(y >= E.numrows) {   
        if (E.numrows == 0 && y == E.screenrows / 3) {
            ...
            int welcomelen = snprintf(welcome, sizeof(welcome),
                                      "TXT Editor -- version %s", TXTEDITOR_VERSION);
